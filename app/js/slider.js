export default class Slider {
  constructor (
	...
	this.showOnMoblieDevices = showOnMoblieDevices
  }
  ...
  _activeDot () {
    if (this._dots !== undefined && this.dots === true) {
      const items = [...this._dots.children]
      items.forEach((d) => {
  ...
  _hideOnMobileDevices (){
	let root = this.getRootContainer();
	if (!this.showOnMoblieDevices && window.innerWidth < 500){
  ...
  show () {
	...
	this._addListener(window, 'resize', this._hideOnMobileDevices)
